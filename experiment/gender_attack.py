...
_, test_loader = get_Dataloader('gender', root=root, batch_size=batch_size, transform=transform, shuffle_flag=True)
...
for attack_method, attack_method_str in zip(attack_methods, attack_methods_str):
    ...
    for step in steps:
        ...
        attack = attack_method(model, steps=step, lr=0.01)
        correct = 0
        total = len(test_loader.dataset)
        flag = True
        for images, labels in test_loader:
            ...
            if flag:
                # print(eps)
                show_image(torchvision.utils.make_grid(images.cpu().data, normalize=True), [attack_method_str + ' - eps:', str(step) + '\n'] + [pre], True)
                flag = False
        acc = 100 * float(correct) / total
        print('Accuracy of test text: %f %%' % acc,' with using eps: %f' % step)
        accuracy.append(acc)

    time.sleep(1)
    plot_eps_acc(attack_method_str+' performance', steps, accuracy)
